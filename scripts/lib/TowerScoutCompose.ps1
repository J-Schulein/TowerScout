...
$script:TowerScoutCudaPytorchIndexUrl = "https://download.pytorch.org/whl/cu128"
...
function Assert-TowerScoutPackageGpuCompatibility {
    ...
    if ($packageFlavor -eq "cpu") {
        throw "This CPU TowerScout package does not support -Gpu on. Use the CUDA 12.8 package for GPU validation, or launch this CPU package with -Gpu off."
    }
}
