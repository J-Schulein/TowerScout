...
import ts_maps
from flask import Flask, render_template, send_from_directory, request, session, Response, jsonify, abort as flask_abort
...
# Simulated host helper endpoints for local testing only.
# Enable by setting environment variable TOWERSCOUT_SIMULATED_HELPER=1
SIMULATED_HELPER_ENABLED = os.getenv('TOWERSCOUT_SIMULATED_HELPER', '').strip().lower() in ('1', 'true', 'yes', 'on')
SIMULATED_HELPER_OPS = {}

if SIMULATED_HELPER_ENABLED:
    api_logger.info("Simulated host helper endpoints enabled (TEST ONLY)")

    @app.route('/operations/provider-tls-repair', methods=['POST', 'OPTIONS'])
    def simulated_provider_tls_repair():
        # CORS preflight
        ...
        if 'application/json' not in content_type:
            return jsonify({'state': 'rejected_unexpected_field', 'message': 'Content-Type must be application/json'}), 400

        try:
            data = request.get_json(force=True)
        except Exception:
            return jsonify({'state': 'rejected_operation_authorization', 'message': 'Invalid JSON body'}), 400

        if not isinstance(data, dict):
            return jsonify({'state': 'rejected_unexpected_field', 'message': 'Request body must be JSON object'}), 400

        allowed = {'provider', 'confirmation', 'operation_authorization'}
        extra = set(data.keys()) - allowed
        if extra:
            return jsonify({'state': 'rejected_unexpected_field', 'message': 'Unexpected fields', 'fields': list(extra)}), 400

        provider = str(data.get('provider') or '').strip().lower()
        if provider not in ('google', 'azure'):
            return jsonify({'state': 'rejected_unknown_provider', 'message': 'Unknown provider'}), 400

        confirmation = data.get('confirmation')
        if confirmation != 'repair_tls_and_restart':
            return jsonify({'state': 'rejected_confirmation', 'message': 'Invalid confirmation value'}), 400

        op_auth = data.get('operation_authorization')
        if not isinstance(op_auth, str) or not re.match(r'^[A-Za-z0-9_-]{32,128}$', op_auth):
            return jsonify({'state': 'rejected_operation_authorization', 'message': 'Invalid operation_authorization token'}), 400

        # Generate a simulated operation id and record basic state
        operation_id = secrets.token_hex(16)
        SIMULATED_HELPER_OPS[operation_id] = {'state': 'planned', 'polls': 0, 'terminal': False, 'provider': provider}

        response = jsonify({'operation_id': operation_id, 'state': 'planned'})
        response.status_code = 202
        response.headers['Access-Control-Allow-Origin'] = request.headers.get('Origin', '*')
        return response

    @app.route('/operations/<operation_id>', methods=['GET', 'OPTIONS'])
    def simulated_operation_status(operation_id):
        # CORS preflight
        if request.method == 'OPTIONS':
            response = jsonify({'ok': True})
            response.status_code = 204
        ...
        if operation_id not in SIMULATED_HELPER_OPS:
            return jsonify({'state': 'operation_expired'}), 410

        op = SIMULATED_HELPER_OPS[operation_id]
        op['polls'] += 1

        # First poll -> active, subsequent polls -> ready (terminal)
        if op['polls'] == 1:
            op['state'] = 'active'
            op['classification'] = 'active'
            op['terminal'] = False
            return jsonify({'operation_id': operation_id, 'state': 'active', 'classification': 'active', 'terminal': False}), 200

        op['state'] = 'ready'
        op['classification'] = 'terminal_success'
        op['terminal'] = True
        return jsonify({'operation_id': operation_id, 'state': 'ready', 'classification': 'terminal_success', 'terminal': True}), 200
