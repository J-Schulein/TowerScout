...
Hosted asset download is out of scope for the v1 control package. The supported
...
a separate design for asset hosting, checksum enforcement, retries, proxy/TLS
behavior, partial-download recovery, and restricted-network fallback.
...
The launcher reports Compose-provider information before startup. For Podman, a `PODMAN_COMPOSE_PROVIDER` override is checked before Compose is invoked so a mistyped or unapproved provider path fails early with an actionable message. The package includes `scripts\install-podman-compose-provider.cmd` for connected support/setup use; `-Apply` installs the catalog-selected provider into a package-local, provider-only virtual environment and updates only `PODMAN_COMPOSE_PROVIDER` in `.env`. The managed layout retains the exact hash-verified wheels under `wheelhouse`, creates the virtual environment without embedded packaging tools, replaces pip-added package metadata with the exact wheel `RECORD` inventory, rejects packaging bootstrap distributions, `.pth` files, and generated `.pyc` files, and does not create a command wrapper. The current PowerShell path sets `PYTHONDONTWRITEBYTECODE=1` for every provider validation and `podman compose` child. The future native invocation also uses isolated Python (`-I`) with bytecode disabled (`-B`), but that native owner remains unwired. The installer verifies that its caller-selected Python reports the required version and platform, but that compatibility probe is not a trust decision; Gate A's native owner must authenticate and retain the interpreter and provider closure before launcher use. The launcher does not install this dependency implicitly.
