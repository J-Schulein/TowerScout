...
def test_yolov5_detector_reports_dependency_mismatch_before_hub_load():
    with patch('ts_yolov5.os.path.exists', return_value=True), \
         patch(
             'ts_yolov5._validate_runtime_dependencies',
             side_effect=RuntimeError('TowerScout runtime dependency check failed before YOLO initialization.')
         ), \
         patch('ts_yolov5.torch.hub.load') as mock_load, \
         patch('ts_yolov5.torch.cuda.is_available', return_value=False), \
         patch('ts_yolov5.torch.get_num_threads', return_value=4):
        with pytest.raises(ModelLoadError) as exc_info:
            YOLOv5_Detector('newest.pt')
...
def test_validate_runtime_dependencies_reports_missing_required_hub_imports():
...
def test_yolov5_detector_disables_autoinstall_during_hub_load():
    recovered_model = Mock()
    ...
    with patch.dict(os.environ, {YOLO_AUTOINSTALL_ENV_VAR: 'true'}, clear=False), \
         patch('ts_yolov5.os.path.exists', return_value=True), \
         patch('ts_yolov5._validate_runtime_dependencies'), \
         patch('ts_yolov5.torch.hub.load', side_effect=fake_hub_load), \
         patch('ts_yolov5.torch.cuda.is_available', return_value=False), \
         patch('ts_yolov5.torch.get_num_threads', return_value=4):
        detector = YOLOv5_Detector('newest.pt')
