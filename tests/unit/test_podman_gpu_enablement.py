...
def test_podman_gpu_image_uses_package_template_digest_before_env_exists():
    ...
    (temp_root / ".env.example").write_text(
        "\n".join(
            [
                "TOWERSCOUT_IMAGE=ghcr.io/j-schulein/towerscout:v0.1.0-ga-cuda128",
    ...
    try:
        ...
        $expected = "ghcr.io/j-schulein/towerscout:v0.1.0-ga-cuda128@{digest}"
        ...
        Set-Content -LiteralPath "{temp_root / '.env'}" -Encoding ASCII -Value @(
            "TOWERSCOUT_IMAGE=ghcr.io/j-schulein/towerscout:local-cuda128",
            "TOWERSCOUT_IMAGE_DIGEST="
        )
        $resolvedEnvWithoutDigest = Get-TowerScoutPodmanGpuImage
        if ($resolvedEnvWithoutDigest -ne "ghcr.io/j-schulein/towerscout:local-cuda128") {{
            throw ".env image should not append .env.example digest: $resolvedEnvWithoutDigest"
        }}

        Set-Content -LiteralPath "{temp_root / '.env'}" -Encoding ASCII -Value @(
            "TOWERSCOUT_IMAGE=ghcr.io/j-schulein/towerscout:local-cuda128@{digest}",
            "TOWERSCOUT_IMAGE_DIGEST=sha256:{'c' * 64}"
        )
        $resolvedEnv = Get-TowerScoutPodmanGpuImage
        if ($resolvedEnv -ne "ghcr.io/j-schulein/towerscout:local-cuda128@{digest}") {{
            throw ".env should win and should not append a second digest: $resolvedEnv"
        }}

        $env:TOWERSCOUT_IMAGE = "ghcr.io/j-schulein/towerscout:process-cuda128"
        $env:TOWERSCOUT_IMAGE_DIGEST = "sha256:{'d' * 64}"
        $resolvedProcessEnv = Get-TowerScoutPodmanGpuImage
        if ($resolvedProcessEnv -ne "ghcr.io/j-schulein/towerscout:process-cuda128@sha256:{'d' * 64}") {{
