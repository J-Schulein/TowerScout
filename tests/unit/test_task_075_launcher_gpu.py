...
def test_gpu_launcher_helpers_use_cpu_safe_auto_and_build_indexes():
    ...
    if ($env:PYTORCH_INDEX_URL -ne "https://download.pytorch.org/whl/cu128") {{
        throw "Expected -Gpu auto -Build to select the CUDA PyTorch index."
    }}
    if ($env:TOWERSCOUT_PYTORCH_FLAVOR -ne "cuda128") {{
        throw "Expected -Gpu auto -Build to set TOWERSCOUT_PYTORCH_FLAVOR=cuda128."
    }}
...
def test_cpu_release_package_rejects_gpu_on_without_blocking_build_or_cuda_package():
    ...
    try:
        ...
        try {{
            ...
            if ($_.Exception.Message -notmatch "CPU TowerScout package" -or $_.Exception.Message -notmatch "CUDA 12.8 package") {{
                throw
            }}
        }}
        ...
        Set-Content -LiteralPath "{manifest_path}" -Encoding ASCII -Value '{{"release_version":"v0.1.0-ga-cuda128","pytorch_flavor":"cuda128"}}'
        Set-TowerScoutGpuEnvironment -Gpu on
        if ($env:TOWERSCOUT_DEVICE -ne "cuda") {{
