...
def test_compose_helper_initializes_env_from_package_template_without_overwrite():
    ...
    env_example.write_text(
        "TOWERSCOUT_IMAGE=ghcr.io/j-schulein/towerscout:v0.1.0-rc1-cuda128\n",
        encoding="utf-8",
    )
...
def test_package_env_image_mismatch_fails_closed_without_overwrite():
    ...
    (temp_root / ".env").write_text(
        "\n".join(
            [
                "TOWERSCOUT_IMAGE=ghcr.io/j-schulein/towerscout:old-cuda128",
    ...
    try:
        ...
        if ($envText -notmatch "old-cuda128" -or $envText -notmatch "PODMAN_COMPOSE_PROVIDER") {{
...
def test_bootstrap_image_reference_uses_digest_pin_from_env_example():
    ...
    try:
        ...
        if ($image -ne "ghcr.io/j-schulein/towerscout:v0.1.0-rc1-cuda128@{digest}") {{
