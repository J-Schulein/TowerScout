...
Certificate apply, execution/cleanup integration, and the legacy transaction
refactor are still open.
After Gate A source
...
| `TASK-087` Host-Side TLS Repair Control Plane | In progress / IMPLEMENT at implementation head `8a6dd47`; `4ced15d` is the validated exact head. Native rollback and startup admission are composed, and the exact target plus ordered lock/recovery context are retained through confirmation. Durable forward-journal/provider linkage and exact certificate candidate staging/verification are checkpointed; certificate apply, execution/cleanup integration, the `repair.py` refactor, live trust/runtime proof, and final validation remain. PR #67 remains Draft. After Gate A acceptance and the merge decision, complete Tasks 096 and 102 before Task-087 Gate B normal-package integration. |
