...
| 5 | `TASK-073` Clean-Machine Pilot / UAT Execution Plan | NOT_STARTED | B/C (User Testing / Release Validation) | 0.5-1 day (4-8h) | `TASK-066`; draft user package docs; owner-accepted route-test gap disposition | Add and keep. Define pilot tester instructions, acceptance checklist, environment capture, friction logging, issue-report workflow, success criteria, and support escalation before asking end users to test. Start after the `TASK-066` Flask route-test timeout/isolation gap is fixed or explicitly accepted as an internal-only risk. |
...
| 9 | `TASK-067` CI Release Gate Tightening | NOT_STARTED | C (CI / Release Engineering) | 1-2 days (8-16h) | `TASK-066` checklist | Keep. Once the release-candidate checklist exists, promote the highest-value digest, manifest, packaging, timeout, docs/license route, and launcher checks into CI or documented required/manual gates. Also remove or archive the stale legacy `AGENTS.md/` guidance after preserving any unique current-value content. Keep scope narrow and release-protective. |
| 10 | `TASK-068` Windows Test Portability And Script Validation | NOT_STARTED | B/C (Testing / Developer Experience) | 0.5-1 day (4-8h) | `TASK-065`; current release helper tests; `TASK-066` route-test finding | Keep as a release-support follow-up. Decide whether PowerShell release-helper coverage stays Windows-only or gains explicit PowerShell Core coverage in CI, and isolate Flask route tests from local `.env`, logs, uploads, sessions, and cache paths. |
| 11 | `TASK-075` Single GPU-Capable Package Implementation | ACTIVE | C (Runtime Policy / Hardware Compatibility / Release Packaging) | 1-3 days (8-24h) | Current CPU baseline; `TASK-051` CUDA audit; `TASK-079` single-package plan | Active in Sprint 06. Start with shared `TOWERSCOUT_DEVICE` policy resolution, EfficientNet memory-bound CUDA chunking, readiness diagnostics, and fixed-fixture parity checks; keep the default launch CPU-safe, add optional GPU launch overlay, validate CPU fallback on non-GPU hosts, and validate NVIDIA Docker Desktop WSL2 before claiming GPU support. |
...
`TASK-077` has a narrow compliance-payload slice in Sprint 06. `TASK-074`, `TASK-067`, and `TASK-068` are high-value follow-through if capacity remains or if `TASK-066` exposes friction that should be automated before external testing. The May 27, 2026 route-test timeout/isolation finding is a concrete candidate for `TASK-067`/`TASK-068` before broad `TASK-073` pilot prep. The stale legacy `AGENTS.md/` guidance should be removed or archived in the next cleanup PR to avoid future agent confusion.
