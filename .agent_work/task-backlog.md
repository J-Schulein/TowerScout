...
| 8 | `TASK-085` Dataset ZIP Restore Path Traversal Hardening | NOT_STARTED | C (Security / Dataset Restore Hardening) | 0.5-1 day (4-8h) | Current `/uploaddataset` restore path and dataset export/restore tests; `TASK-084` final package sequencing | Add as a focused security hardening follow-up from RC5 review and treat as a hard pre-final-package gate unless dataset restore is disabled or explicitly excluded. The dataset upload route validates the uploaded ZIP file but still derives restored member paths from ZIP entry names and writes them under the session temp directory without rejecting `..`, absolute paths, backslashes, drive prefixes, or resolved paths outside the intended temp root. Patch restore extraction to normalize member names, reject unsafe paths before writing, preserve valid exported dataset restore behavior, and add traversal regression tests. |
