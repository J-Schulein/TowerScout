...
| 11 | `TASK-075` Single GPU-Capable Package Implementation | NOT_STARTED | C (Runtime Policy / Hardware Compatibility / Release Packaging) | 1-3 days (8-24h) | Current CPU baseline; `TASK-051` CUDA audit; `TASK-079` single-package plan | Move forward from the selected single CUDA-capable package/image direction. Keep the default launch CPU-safe, add optional GPU launch overlay and `TOWERSCOUT_DEVICE` controls, validate CPU fallback on non-GPU hosts, and validate NVIDIA Docker Desktop WSL2 before claiming GPU support. |
