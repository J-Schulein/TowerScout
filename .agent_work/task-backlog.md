...
| 3 | `TASK-071` End-User Release Package Documentation | COMPLETED | B/C (Documentation / User Enablement) | 1-2 days (8-16h) | `TASK-069`; `TASK-072`; release package shape | Completed for Task-071 scope. Package-local quick start, package guide, user guide, Project Overview, Resource Links, and release package/image integration are validated and ready for `TASK-066`. |
...
| 7 | `TASK-076` Provider API Key Exposure And Restriction Policy | POLICY_DECISION_RECORDED | C (Security / Release Policy) | 0.5-1.5 days (4-12h) | Current setup/settings and provider-loading behavior; `TASK-069` alignment; `TASK-071`; `TASK-066` | Active in the Sprint 06 policy lane. V1 RC1 accepts browser-visible map SDK keys only for the local pilot with site/user-owned restricted keys, provider-side quotas/billing alerts/monitoring/rotation, and clear user guidance. Unrestricted shared project keys are unsupported and would become an engineering blocker unless explicitly risk-accepted. |
...
5. `TASK-076` to apply the recorded site/user-owned restricted-key policy in `TASK-071` docs and confirm it during `TASK-066` before broadening distribution.
