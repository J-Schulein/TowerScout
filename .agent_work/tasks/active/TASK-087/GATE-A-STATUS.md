...
designed.
**Local Checkpoint**: Committed checkpoints `e9a8559` and `ab5aefa` extend the
reviewed certificate/provider mutation composition through durable exact
runtime stop/start. The old authenticated container is removed only by exact ID;
the repaired profile is twice captured absent, started with a fixed service-only
command, and rebound only after unchanged image/all-volume proof and changed
container evidence. The expanded runtime/repair ring passes `371/371`; isolated
strict typing, Black, blocking lint, focused Bandit, compilation, and diff
checks pass. Terminal verification/commit, cleanup, recovery-on-failure
composition, and the legacy transaction replacement remain open; launcher
...
| 7 | Transaction refactor | **PARTIAL** | Checkpoint `9d7f533` defines and persists the strict forward state sequence from certificate staging through commit/cleanup, with exact provider-generation linkage and fresh-process discovery. Checkpoints through `4b9a44d` compose exact rollback arming and certificate/provider mutation. Checkpoints `e9a8559` and `ab5aefa` remove only the old exact container, twice verify repaired-profile absence, start only the fixed service, and rebind exact authority through durable `runtime_started`; the path remains unreachable from the launcher. The older prototype transaction remains a behavior reference only. | Compose terminal verification, commit, exact cleanup, and recovery-on-failure behind confirmation; then replace the legacy `repair.py` transaction while enforcing pre-write/pre-restart/terminal revalidation. |
...
Continue from certificate-temp-cleanup checkpoint `244eeb7`. Refactor `repair.py`
