...
**Implementation Head**: `9bdf51c`
**Validated Exact Head**: `9873c42`
**Remote Exact-Head Status**: CI/CD run `35364176635`, Task-087 run
`35364176462`, and Trivy passed at `9873c42`; the main-only build was neutral as
designed.
**Local Checkpoint**: Committed checkpoint `9bdf51c` applies both exact verified
certificate candidates through the retained exact Docker/Podman target only
after independently reconciling the authenticated forward pointer and matching
the rollback original-state record, candidate plan, target token, and package
identity. It retains both protected-root source identities while copying,
accepts only the authenticated original/absence or already-applied candidate,
uses fixed atomic container commands with file and directory flushes, verifies
both destinations and stage absence, and only then persists
`certificates_applied`. The affected retained-target/recovery/journal ring
passes `337/337`; focused static/security gates pass. Forward temp cleanup and
the remaining execution integration are still absent, so launcher mutation
remains disabled.
...
Continue from certificate-apply checkpoint `9bdf51c`. Add authenticated forward-
temp cleanup primitives, then refactor `repair.py`
