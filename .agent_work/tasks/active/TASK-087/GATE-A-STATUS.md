...
**Implementation Head**: `6bc37b3`
**Validated Exact Head**: `eb76746`
**Remote Exact-Head Status**: CI/CD run `35276303269`, Task-087 run
`35276303330`, and Trivy passed at `eb76746`; the Task-087 Windows helper job's
first timing-only failure passed on its exact failed-job rerun, and the main-only
build was neutral as designed.
**Local Candidate**: Pushed certificate rollback-candidate checkpoint
`6bc37b3`. It derives the exact local-CA and combined-bundle bytes from the
selected Windows root plus system bundle, then records strict hash/size/`0644`
summaries in authenticated generation 1 before any backup artifact or mutation.
Production certificate application/restoration and exact-head workflows remain
open; runtime mutation is disabled.
...
Continue from pushed certificate-authority checkpoint `6bc37b3`. First add
exact missing-container recreation and implement certificate application/
...
Planning after `6bc37b3` remains based on fixed acceptance criteria rather than
