...
**Implementation Head**: `ee76805`
**Validated Exact Head**: `88d29f2`
**Remote Exact-Head Status**: CI/CD run `35358255080`, Task-087 run
`35358255088`, and Trivy passed at `88d29f2`; the main-only build was neutral as
designed.
**Local Checkpoint**: Committed checkpoint `ee76805` admits authenticated pending
repair recovery after the single-instance lock and before ordinary launcher
startup. Recovery success is reported with fixed text; unresolved recovery
blocks the app with fixed text and exit code 3. Focused launcher/startup/recovery
tests pass `60/60`, and focused static/security gates pass.
Forward mutation remains disabled.
...
Continue from startup-admission checkpoint `ee76805`, push it with this ledger,
and require exact-head workflows. Then refactor `repair.py` around the immutable
target and durable manager only after the forward-journal and terminal-provider
linkage boundaries are explicit.
