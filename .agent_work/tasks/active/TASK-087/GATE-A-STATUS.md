...
**Implementation Head**: `4b9a44d`
**Validated Exact Head**: `8fd5806`
**Remote Exact-Head Status**: CI/CD run `35372618247`, Task-087 run
`35372618295`, and Trivy passed at `8fd5806`; the main-only build was neutral as
designed.
**Local Checkpoint**: Committed checkpoint `4b9a44d` composes the reviewed exact
certificate apply/temp cleanup and provider environment stage/promotion ports
under the retained transaction context. It durably reaches
`certificates_applied`, mirrors all four authenticated provider mini-journal
generations, and reaches `environment_applied` without adding a launcher call
site. Focused composition tests pass `16/16`; isolated strict typing, Black,
blocking lint, focused Bandit, and diff checks pass. Runtime transitions,
terminal verification/commit, cleanup, recovery-on-failure composition, and the
legacy transaction replacement remain open; launcher mutation remains disabled.
...
| 7 | Transaction refactor | **PARTIAL** | Checkpoint `9d7f533` defines and persists the strict forward state sequence from certificate staging through commit/cleanup, with exact provider-generation linkage and fresh-process discovery. Checkpoints through `1c59252` compose exact rollback arming, certificate staging, and provider linkage. Checkpoint `4b9a44d` composes certificate apply/temp cleanup and provider `.env` stage/promotion through durable `environment_applied` under the retained context, but remains unreachable from the launcher. The older prototype transaction remains a behavior reference only. | Compose runtime transition, terminal verification, commit, exact cleanup, and recovery-on-failure behind confirmation; then replace the legacy `repair.py` transaction while enforcing pre-write/pre-restart/terminal revalidation. |
