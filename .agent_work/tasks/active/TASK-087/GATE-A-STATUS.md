...
**Validated Exact Head**: `050dc5c399e2900a26cbd04ef1c1a6527433b5c7`
**Local Candidate**: Pure protected-root-owned metadata-pointer load,
classification, and missing/stale repair orchestration through an injected port
...
run `35014300773`, Task-087 run `35014300816`, and Trivy. Independently reviewed
...
and Trivy; the main-only build was neutral as designed. A local candidate now
implements pure pointer-aware load and ensure/repair orchestration above the
native generation-file adapter; native pointer replacement and recovery work
remain open. Initial independent source/security review returned `CLEAN/PASS`
with no actionable Low-or-higher findings.
...
DACL and flush/reopen verification. The local candidate adds only pure
pointer-aware load, classification, and missing/stale repair orchestration over
an injected pointer port. Native pointer-file replacement, indeterminate-result
...
3. **Build durable recovery and refactor repair (slices 6-7).** Review and
  use the checkpointed native generation adapter and local pure pointer policy
  to add native durable pointer replacement, backups, and the fresh-process
  journal/recovery manager before enabling the refactored transaction.
...
Implement the native same-directory pointer temp/create/DACL/write/flush/reopen,
close-before-rename `MoveFileExW(REPLACE_EXISTING | WRITE_THROUGH)`, and exact
destination verification adapter beneath the pure pointer policy before any
production staging or destination promotion is wired.
