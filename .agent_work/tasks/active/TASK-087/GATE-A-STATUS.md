...
**Implementation Head**: `9d7f533`
**Validated Exact Head**: `b2369bb`
**Remote Exact-Head Status**: CI/CD run `35358921493`, Task-087 run
`35358921527`, and Trivy passed at `b2369bb`; the main-only build was neutral as
designed.
**Local Checkpoint**: Committed checkpoint `9d7f533` adds the authenticated
forward repair journal and native durable storage namespace. The forward stream
is anchored to the exact `rollback_armed` generation, each provider write-ahead
prefix binds the exact provider mini-journal generation, and fresh-process
classification retains that owned stream for rollback. Focused tests pass
`63/63`; the broader selected ring passes `700/700` in a fresh external temp
root, and focused static/security gates pass. Certificate forward staging/apply
and execution/cleanup integration remain absent, so forward mutation remains
disabled.
...
| 7 | Transaction refactor | **PARTIAL** | Checkpoint `9d7f533` defines and persists the strict forward state sequence from certificate staging through commit/cleanup, with exact provider-generation linkage and fresh-process discovery. It does not yet execute certificate staging/apply, runtime transitions, commit, or cleanup. The older prototype transaction remains a behavior reference only. | Implement the certificate forward primitives and cleanup owner, then refactor `repair.py` to consume the immutable resolved target, forward journal, and recovery manager; remove process-memory-only backup and unchecked rollback; enforce pre-write/pre-restart/terminal revalidation. |
...
Continue from forward-journal checkpoint `9d7f533`. Add the certificate forward
staging/apply and authenticated cleanup primitives, then refactor `repair.py`
