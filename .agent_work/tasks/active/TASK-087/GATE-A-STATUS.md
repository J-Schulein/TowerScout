...
designed.
**Local Checkpoint**: Committed checkpoint `b2f6458` admits authenticated pending
repair recovery before a new repair, handles retained-present or exact-absent
targets under the same ordered mutex pair, transfers all retained authorities
into the durable native manager, and requires a clean rescan. Tests pass
`226/226` focused, `849/849` broad non-native, and `245/245` isolated Windows-
native; all focused static/security gates pass.
The app is not wired to this front door and forward mutation remains disabled.
...
Continue from recovery-front-door checkpoint `b2f6458`, push it with this
ledger, and require exact-head workflows. Then wire launcher recovery admission
and refactor `repair.py` around the immutable target and durable manager only
after the forward-journal and terminal-provider linkage boundaries are explicit.
