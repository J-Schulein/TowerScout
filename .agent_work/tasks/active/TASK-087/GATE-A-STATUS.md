...
**Implementation Head**: `080fef8`
**Validated Exact Head**: `01d2a96`
**Remote Exact-Head Status**: CI/CD run `35373767689`, Task-087 run
`35373767723`, and Trivy passed at `01d2a96`; the main-only build was neutral as
designed.
**Local Checkpoint**: Committed checkpoints through `080fef8` compose the full
durable repair path: exact runtime stop/start, terminal target/environment/
certificate/readiness/provider verification, commit, exact encrypted-backup
cleanup, same-session rescan, and recovery-on-failure. Incomplete repairs resume
rollback; committed cleanup failures resume cleanup and cannot roll back a
successful repair. Paired terminal forward/rollback journal history is
suppressed in both directions. The selected Gate A runtime/recovery/repair ring
passes `1818/1818`; strict typing, Black, blocking lint, focused Bandit,
compilation, and diff checks pass. The legacy `repair.py`/confirmation call-site
replacement remains open; launcher mutation remains disabled.
...
1. **Integrate the production call site (slices 5-7).** Make `repair.py` and
   confirmation consume the complete native coordinator and its fresh-process
   recovery behavior.
2. **Remove the legacy transaction (slice 7).** Make `repair.py` consume the
...
Continue from coordinator checkpoint `080fef8`. Refactor `repair.py` and the
confirmation call site around the immutable target, durable forward journal,
and rollback manager. Preserve mutation-off until the integrated path is
complete and reviewed. Retry slice
