...
**Implementation Head**: `d1494f6`
**Validated Exact Head**: `076d566`
**Remote Exact-Head Status**: CI/CD run `35294378326`, Task-087 run
`35294378346`, and Trivy passed at `076d566`; the main-only build was neutral as
designed.
**Local Candidate**: Committed checkpoint `d1494f6` captures and retains the
duplicated package-root authority, protected recovery root, authenticated
cross-protocol recovery scan, and environment-then-target mutex pair through
the confirmation lifetime. Provider recovery blocks before target-lock
acquisition; repair recovery remains retained evidence. Integrated repair
...
| 6 | Recovery manager | **PARTIAL** | The authenticated journal, pointer protocol, exact backup/restore chain, fresh-process resumption, terminal cleanup, retained/recreated runtime availability, certificate restoration, volume-preserving restart, and terminal verification authority are checkpointed through `515c1f5`. Checkpoint `f298a3e` adds the terminal native verifier. Checkpoint `2266f84` composes every native rollback port under one retained protected root, reconstructs certificate identity from authenticated state, and safely supports post-arm failure before a provider stream exists. Checkpoint `6ff351c` exposes the retained package-root only through the exact target owner and brackets each callback with complete target revalidation. Checkpoint `d1494f6` retains the identity-matched package-root lease, protected root, recovery scan, and ordered environment/target mutexes through confirmation. Its focused evidence passes `16/16`, its affected broad ring passes `421/421`, and the isolated native DACL proof passes `1/1`; static/security gates pass. | Exercise the complete manager through the retained transaction and add the absent-target fresh-process recovery front door. |
...
Continue from ordered-context checkpoint `d1494f6`. Add the absent-target
fresh-process recovery front door and transfer the retained target/context into
the durable recovery/repair executor, then refactor `repair.py` around that
immutable target and manager. Preserve mutation-off until the integrated path
is complete and reviewed. Retry slice
