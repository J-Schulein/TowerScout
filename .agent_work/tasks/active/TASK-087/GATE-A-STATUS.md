...
designed.
**Local Checkpoint**: Committed checkpoint `244eeb7` adds the retry-safe cleanup
owner after `9bdf51c` durably applies both exact certificate candidates. Cleanup
reconciles the authenticated four-generation forward pointer, revalidates the
recorded host identities, DACL, sizes, and hashes under the protected root,
accepts exact absence, and deletes no substituted file. The affected retained-
target/recovery/journal ring passes `339/339`; focused static/security gates
