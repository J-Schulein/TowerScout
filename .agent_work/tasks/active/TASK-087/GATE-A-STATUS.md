...
1. **Finish trust-backed exact-target wiring (slices 2-4).** The native
   ...
   process-environment, runtime/endpoint, and acceleration authorities. A local
   production facade now connects those inputs through Windows trust and native
   observation to a still-held exact target at independently reviewed checkpoint
   `db7aee8`. Next, make confirmation consume the exact owner and wire
