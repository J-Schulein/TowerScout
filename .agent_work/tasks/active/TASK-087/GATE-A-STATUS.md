...
**Implementation Head**: `95ca37d68e2dd5039a28766cba0df51303f93ff4`
**Validated Exact Head**: `145e0b91cca28da7f08c9a57eb07de860848f950`
**Local Candidate**: Independently reviewed same-call pointer completed-move
reconciliation; no cleanup, recovery action, or runtime mutation
...
neutral as designed. A local independently reviewed source candidate accepts an
ordinary move API error as success only after same-call source-absence and exact
destination identity/DACL/path/size/byte proof. Restart classification, durable
temp-identity binding, and cleanup remain open. Its focused, adjacent, and full
launcher tests pass `39/39`, `105/105`, and `1647/1647`; initial independent
...
exact destination verification. A local independently reviewed candidate adds
same-call completed-move reconciliation after an ordinary API error. Durable
restart classification and temp-identity binding, backup, recovery, cleanup,
...
3. **Build durable recovery and refactor repair (slices 6-7).** Review and
  use the checkpointed native generation adapter and pointer policy plus the
  checkpointed native pointer adapter and reviewed same-call reconciliation to
  add durable authenticated temp-identity binding, restart classification,
  backups, and the fresh-process journal/recovery manager before enabling the
  refactored transaction. Add exact cleanup only after that binding exists.
...
Add explicit indeterminate move-result reconciliation and exact journal-bound
pointer-temp cleanup beneath the pure pointer policy before any production
staging or destination promotion is wired.
