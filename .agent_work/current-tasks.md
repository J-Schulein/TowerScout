...
- 2026-05-07 GHCR workflow follow-up identified: branch-triggered artifact name was `image-metadata-` because the workflow used `inputs.tag` on a `push` event; local patch now uses `steps.build.outputs.tag` and needs push/run confirmation
