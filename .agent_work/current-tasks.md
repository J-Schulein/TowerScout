...
**Status**: IN_PROGRESS - runtime cleanup, shared asset-bundle identity, package guardrails, Podman provider onboarding, user/support docs, RC6 CPU/CUDA image publication, RC6 package generation, Docker CPU smoke, CUDA CPU-fallback smoke, and locally patched Podman CPU smoke passed; `TASK-085` is merged/validated; the Podman CPU smoke exposed a source/package rebuild blocker for dotted release package names; committing that fix, rebuilding/retesting publishable ZIPs, GPU-host validation/hold decision, and sanitized release evidence remain
