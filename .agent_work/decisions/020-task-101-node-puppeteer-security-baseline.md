...
- Task-087 remains paused. PR #72 and default-branch dependency reconciliation
  later passed, PR #73 recorded the checkpoint, and current `main` through
  `9276084` is now semantically integrated into PR #67. This decision still
  does not resume Task-087. Resumption requires green checks at that branch's
  new exact head followed by an explicit governance transition.
...
passed. The current-main integration is present in PR #67, while exact-head
validation and the explicit lifecycle transition remain the separate acceptance
gates recorded in Task-101.
