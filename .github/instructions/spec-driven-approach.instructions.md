...
## Canonical `.agent_work` Organization

This section is the authoritative `.agent_work` organization contract. If any older example later in this file conflicts with this section, this section wins.

- Keep current sprint task docs in `.agent_work/tasks/active/`, even when a task is already marked `COMPLETED`.
- Move the finished sprint batch from `.agent_work/tasks/active/` to `.agent_work/tasks/completed/` during sprint closeout.
- Do not leave loose task markdown files at `.agent_work/tasks/` root.
- Keep project-wide ADRs only in `.agent_work/decisions/`, using numeric filenames.
- Keep task-local decision memos, proof docs, raw evidence, and helper scripts with the owning task.
- Keep `.agent_work/context/guides/` for evergreen reference, `.agent_work/context/analysis/` for cross-task narrative analysis only, and `.agent_work/context/status/` for active/current sprint status docs only.
- Archive superseded status docs, drafts, and historical snapshots under `.agent_work/context/archive/YYYY-MM/`.
- Treat `.agent_work/tmp/` and `.agent_work/pytest-temp/` as scratch only, never as durable documentation.
- Run `python .agent_work/scripts/validate_agent_work.py` after structure changes or sprint closeout.
